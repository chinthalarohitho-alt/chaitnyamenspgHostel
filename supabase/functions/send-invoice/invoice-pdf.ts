...
export async function buildInvoicePdf(
  ...
  const textR = (s: string, xRight: number, y: number, size = 10, font = reg, color = INK) => {
    const w = font.widthOfTextAtSize(s ?? "", size);
    page.drawText(s ?? "", { x: xRight - w, y, size, font, color });
  };

  const rule = (y: number, x1 = M, x2 = A4.w - M) =>
  ...
  text("Issued", M, y, 8.5, bold, MUTED);
  text(issued, M, y - 15, 10.5);
  text("Status", A4.w / 2, y, 8.5, bold, MUTED);
  const status = opts.paid ? "Token received" : "Awaiting token";
  text(status, A4.w / 2, y - 15, 10.5, bold, opts.paid ? rgb(0.11, 0.66, 0.32) : ORANGE);
  ...
  y -= 17;
  text(b.name, M, y, 11.5, bold);
  y -= 15;
  text(b.phone, M, y, 10, reg, MUTED);
  if (b.email) { y -= 14; text(b.email, M, y, 10, reg, MUTED); }
  if (b.workplace) { y -= 14; text(b.workplace, M, y, 10, reg, MUTED); }
  ...
  // ---------- booking table ----------
  text("DETAILS", M, y, 8.5, bold, MUTED);
  textR("VALUE", A4.w - M, y, 8.5, bold, MUTED);
  y -= 8;
  rule(y);
  y -= 20;
  ...
  for (const [k, v] of rows) {
    text(k, M, y, 10, reg, MUTED);
    textR(v, A4.w - M, y, 10, bold);
    y -= 13;
    rule(y + 4);
    y -= 9;
  }
  ...
  if (amt > 0) {
    page.drawRectangle({
      ...
      color: rgb(0.996, 0.965, 0.941),
      borderColor: rgb(1, 0.851, 0.761), borderWidth: 1,
    });
    text("BOOKING TOKEN", M + 16, y - 12, 8.5, bold, ORANGE);
    text(
      opts.paid ? "Received — thank you" : "Payable to hold your bed",
      M + 16, y - 28, 9.5, reg, MUTED,
    );
    textR(`Rs. ${amt.toLocaleString("en-IN")}`, A4.w - M - 16, y - 24, 19, bold, INK);
    y -= 74;

    if (opts.utr) {
      text(`UPI reference: ${opts.utr}`, M, y, 9, reg, MUTED);
      y -= 18;
    }
  }
  ...
  // ---------- footer ----------
  rule(M + 64);
  text(h.name, M, M + 48, 9.5, bold);
  let fy = M + 35;
  for (const line of wrap(h.address, reg, 8.5, A4.w - M * 2)) {
    text(line, M, fy, 8.5, reg, MUTED);
    fy -= 11;
  }
  text(`Phone: ${h.phone}`, M, fy - 2, 8.5, reg, MUTED);
  textR("Computer generated — no signature required.", A4.w - M, M + 12, 8, reg, LINE);
